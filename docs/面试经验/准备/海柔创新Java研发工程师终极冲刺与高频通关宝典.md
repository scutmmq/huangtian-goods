...
- [模块五：AI 数字员工与 RAG 落地全流程全景二十问（前沿工程护城河）](#模块五ai-数字员工与-rag-落地全流程全景二十问前沿工程护城河)
  - [Q18：向量检索是直接去向量库一个个比对拿 Top-K 吗？（Flat 暴力扫描 vs HNSW 近似检索）](#q18向量检索是直接去向量库一个个比对拿-top-k-吗flat-暴力扫描-vs-hnsw-近似检索)
  - [Q19：向量检索的 HNSW 索引底层原理是什么？（跳表与立体图导航）](#q19向量检索的-hnsw-索引底层原理是什么跳表与立体图导航)
  - [Q20：为什么使用多路混合检索？分开说说 Dense 向量与 Sparse BM25 的优缺点](#q20为什么使用多路混合检索分开说说-dense-向量与-sparse-bm25-的优缺点)
  - [Q21：多路混合检索的 RRF（倒数排名融合）算法原理与公式](#q21多路混合检索的-rrf倒数排名融合算法原理与公式)
  - [Q22：为什么需要 Cross-Encoder Rerank？单塔交叉编码与双塔 Embedding 的本质区别](#q22为什么需要-cross-encoder-rerank单塔交叉编码与双塔-embedding-的本质区别)
  - [Q23：半精度存储（halfvec / FP16）底层原理是什么？会影响准确率和召回率吗？](#q23半精度存储halfvec--fp16底层原理是什么会影响准确率和召回率吗)
  - [Q24：RAG 端到端 6 步/7 步检索流水线全流程时序](#q24rag-端到端-6-步7-步检索流水线全流程时序)
  - [Q25：你的项目有没有做过 RAG 调优？如何进行量化评测？（RAGAS 四大指标）](#q25你的项目有没有做过-rag-调优如何进行量化评测ragas-四大指标)
  - [Q26：如果评测发现指标低，工程上具体如何针对性提升？（对症下药工程闭环）](#q26如果评测发现指标低工程上具体如何针对性提升对症下药工程闭环)
  - [Q27：动态 Nonce 知识隔离沙箱是什么？解决了什么安全问题？（防提示词注入）](#q27动态-nonce-知识隔离沙箱是什么解决了什么安全问题防提示词注入)
  - [Q28：什么是 YAML 案例回归（EvalRunner）？为什么类似于单元测试？](#q28什么是-yaml-案例回归evalrunner为什么类似于单元测试)
  - [Q29：长期记忆（Long-Term Memory）与短期记忆（Short-Term Memory）如何落地？](#q29长期记忆long-term-memory与短期记忆short-term-memory如何落地)
  - [Q30：Workflow（工作流）和 Agent（智能体）的本质区别是什么？](#q30workflow工作流和-agent智能体的本质区别是什么)
  - [Q31：Function Calling、Tool、Skill、MCP（Model Context Protocol）的区别与联系](#q31function-callingtoolskillmcpmodel-context-protocol的区别与联系)
  - [Q32：介绍一下 LangGraph 框架与 Agent 有向图编排机制](#q32介绍一下-langgraph-框架与-agent-有向图编排机制)
  - [Q33：在实际开发/实习中，给你一个需求，如何使用 Vibe Coding 高效完成？](#q33在实际开发实习中给你一个需求如何使用-vibe-coding-高效完成)
  - [Q34：置信度评估与三级动态分流策略（从智能问答到 ITIL 工单流转）](#q34置信度评估与三级动态分流策略从智能问答到-itil-工单流转)
  - [Q35：ITIL 故障工单状态机与 CAS 乐观锁防并发抢单](#q35itil-故障工单状态机与-cas-乐观锁防并发抢单)
  - [Q36：知识库双人审核发布流水线与向量“先写后删”原子替换机制](#q36知识库双人审核发布流水线与向量先写后删原子替换机制)
  - [Q37：AI 数字员工在仓储与运维调度系统中的架构定位与未来演进](#q37ai-数字员工在仓储与运维调度系统中的架构定位与未来演进)
...
### Q19：向量检索的 HNSW 索引底层原理是什么？（跳表与立体图导航）
...
### Q20：为什么使用多路混合检索？分开说说 Dense 向量与 Sparse BM25 的优缺点
...
### Q21：多路混合检索的 RRF（倒数排名融合）算法原理与公式
...
### Q22：为什么需要 Cross-Encoder Rerank？单塔交叉编码与双塔 Embedding 的本质区别
...
### Q23：半精度存储（halfvec / FP16）底层原理是什么？会影响准确率和召回率吗？
...
### Q25：你的项目有没有做过 RAG 调优？如何进行量化评测？（RAGAS 四大指标）
...
### Q26：如果评测发现指标低，工程上具体如何针对性提升？（对症下药工程闭环）
...
### Q27：动态 Nonce 知识隔离沙箱是什么？解决了什么安全问题？（防提示词注入）
...
### Q28：什么是 YAML 案例回归（EvalRunner）？为什么类似于单元测试？
...
### Q29：长期记忆（Long-Term Memory）与短期记忆（Short-Term Memory）如何落地？
...
### Q30：Workflow（工作流）和 Agent（智能体）的本质区别是什么？
...
### Q31：Function Calling、Tool、Skill、MCP（Model Context Protocol）的区别与联系
...
### Q32：介绍一下 LangGraph 框架与 Agent 有向图编排机制
...
### Q33：在实际开发/实习中，给你一个需求，如何使用 Vibe Coding 高效完成？
...
### Q34：置信度评估与三级动态分流策略（从智能问答到 ITIL 工单流转）
...
### Q35：ITIL 故障工单状态机与 CAS 乐观锁防并发抢单
...
### Q36：知识库双人审核发布流水线与向量“先写后删”原子替换机制
...
### Q37：AI 数字员工在仓储与运维调度系统中的架构定位与未来演进
