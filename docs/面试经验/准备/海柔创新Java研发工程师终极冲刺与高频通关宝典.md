...
### Q17：JVM 内存模型、垃圾收集器演进（G1/ZGC）与线上 OOM 排查 SOP

* **内存模型**：线程私有（程序计数器、虚拟机栈、本地方法栈）；线程共享（堆 Heap、元空间 Metaspace、直接内存）。
* **GC 演进动力**：不断压低 STW（Stop The World）停顿时间。G1 划分为 Region，兼顾吞吐与停顿；ZGC 基于读屏障与着色指针实现并发标记与整理，停顿压缩至亚毫秒级。
* **OOM 排查 SOP**：配置 `-XX:+HeapDumpOnOutOfMemoryError`，在线用 `jstat -gcutil` 观测，离线拉取 dump 文件使用 **Eclipse MAT (Memory Analyzer Tool)** 打开，通过支配树（Dominator Tree）定位内存大对象。
